...
def ve_bang_bien_thien(ham_so, bien):
        ...
        if i != 0 and i != len(cac_gia_tri_can_dien_vao_x) - 1:
            if cac_gia_tri_can_dien_vao_x[i] in dao_ham_cap_1_kxd:
                bang_bien_thien.plot(
                    [chieu_rong_cot_ngan + phong_hai_dau + khoang_cach * i - 2, chieu_rong_cot_ngan + phong_hai_dau +
    ...
    for gia_tri in cac_gia_tri_can_dien_vao_x:
        ...
        if gia_tri in ham_so_kxd:
            gia_tri_ham_so.append(gioi_han.tim_gioi_han_hai_phia(ham_so, bien, gia_tri))
    ...
    for i in range(len(cac_dau_can_dien)):
        ...
        if cac_dau_can_dien[i][-1] == '+':
            ...
            bang_bien_thien.arrow(x1, chieu_cao_bang / 5 * 2, x2 - x1, - chieu_cao_bang / 5, head_width=co_font / 3,
