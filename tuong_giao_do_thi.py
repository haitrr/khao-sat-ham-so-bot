...
def tim_toa_do_giao_diem_voi_duong_thang(ham_so, bien, duong_thang):
    loi_giai = huong_dan_giai.LoiGiai('Tìm tọa độ giao điểm của đồ thị hàm số {0} với đường thẳng {1}'.format(
        ...
        xu_ly_chuoi.boc_mathjax("f({0})={1}".format(xu_ly_chuoi.tao_latex(bien), xu_ly_chuoi.tao_latex(duong_thang)))))
    ...
    buoc_1.them_thao_tac(xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(pt_hoanh_do_giao_diem)))
    ...
    buoc_2 = huong_dan_giai.LoiGiai("Tìm nghiệm của phương trình hoành độ giao điểm")

    # In ra pt hoanh do giao diem
    buoc_2.them_thao_tac(xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(pt_hoanh_do_giao_diem)))

    # Chuyển vế phải sang vế trái
    pt_hoanh_do_giao_diem = bat_dang_thuc.bang(pt_hoanh_do_giao_diem.lhs - pt_hoanh_do_giao_diem.rhs, 0)
    ...
    if buoc_2.dap_an:
        ...
        buoc_3 = huong_dan_giai.LoiGiai("Từ các nghiệm xác định tọa độ các giao điểm")

        # Giai thich
        buoc_3.them_thao_tac(
            "Ta xác định giao điểm theo dạng ({0},{1})".format(xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(bien)),
                                                               xu_ly_chuoi.boc_mathjax(
                                                                   "f({0})".format(xu_ly_chuoi.tao_latex(bien)))))

        # In ra cac diem
        cac_diem = [(hoanh, phuong_trinh.tao_ten_ham('f', hoanh)) for hoanh in nghiem]
    ...
    if not buoc_2.dap_an:
        buoc_4.them_thao_tac('Vậy đồ thị hàm số {hs} không cắt đường thẳng {dt}'.format(
            hs=xu_ly_chuoi.boc_mathjax("f({0})={1}".format(xu_ly_chuoi.tao_latex(bien), xu_ly_chuoi.tao_latex(ham_so))),
            dt=xu_ly_chuoi.boc_mathjax(
                "f({0})={1}".format(xu_ly_chuoi.tao_latex(bien), xu_ly_chuoi.tao_latex(duong_thang)))
        ))
    else:
        buoc_4.them_thao_tac('Vậy đồ thị hàm số {hs} cắt đường thẳng {dt} tại các điểm : {diem} '.format(
            hs=xu_ly_chuoi.boc_mathjax("f({0})={1}".format(xu_ly_chuoi.tao_latex(bien), xu_ly_chuoi.tao_latex(ham_so))),
            dt=xu_ly_chuoi.boc_mathjax(
                "f({0})={1}".format(xu_ly_chuoi.tao_latex(bien), xu_ly_chuoi.tao_latex(duong_thang))),
            diem=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_ngoac_nhon(buoc_3.dap_an))
        ))
...
def tim_tham_so_de_ham_so_cat_truc_hoanh_tai_1_diem_duy_nhat(ham_so, bien, tham_so):
    ...
    buoc_2.ten_loi_giai = "Xét trường hợp 1: Hàm số không có cực trị hay " + xu_ly_chuoi.boc_mathjax(
        "f'({0})=0".format(xu_ly_chuoi.tao_latex(bien))) + " có nghiệm kép hoặc vô nghiệm"

    loi_giai.them_thao_tac(buoc_2)

    # Buoc 3
    # Truong hop 2
    buoc_3 = huong_dan_giai.LoiGiai("Xét trường hợp 2: Hàm số có hai cực trị ở cùng phía của trục hoành")
    ...
    buoc_3_1.ten_loi_giai = "Tìm {0} để {1} có hai nghiệm phân biệt".format(
        xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(tham_so)),
        xu_ly_chuoi.boc_mathjax("f'({0})=0".format(
    ...
    buoc_3_2 = huong_dan_giai.LoiGiai("Tìm nghiệm của bất đẳng thức {0}".format(
        xu_ly_chuoi.boc_mathjax(
    ...
    buoc_3_3.them_thao_tac("Tổng hợp kết quả của hai bước trước ta được {0}".format(
        xu_ly_chuoi.boc_mathjax(
            "{0} \in {1}".format(xu_ly_chuoi.tao_latex(tham_so), xu_ly_chuoi.tao_latex(tong_hop_nghiem_buoc_3)))))
    ...
    buoc_4.them_thao_tac("Tổng hợp kết quả của hai trường hợp ta được {0}".format(
        xu_ly_chuoi.boc_mathjax("{0} \in {1}".format(xu_ly_chuoi.tao_latex(tham_so), xu_ly_chuoi.tao_latex(tong_hop)))))

    buoc_4.them_thao_tac("Vậy với {ts} thì đồ thị hàm số cắt trục hoành tại một điểm duy nhất.".format(
        ts=xu_ly_chuoi.boc_mathjax(
            "{0} \in {1}".format(xu_ly_chuoi.tao_latex(tham_so), xu_ly_chuoi.tao_latex(tong_hop)))
    ))
    # Luu ket qua
...
def viet_phuong_trinh_tiep_voi_do_thi_ham_so_tai_mot_diem(ham_so, bien, diem):
    ...
    loi_giai = huong_dan_giai.LoiGiai("Viết phương trình tiếp tuyến với đồ thì hàm số {hs} tại điểm {d}".format(
    ...
    pt = f * (bien - diem) + fx
    buoc_4 = huong_dan_giai.LoiGiai("Phương trình tiếp tuyến cần tìm có dạng {p}".format(
...
def viet_phuong_trinh_tiep_tuyen_voi_do_thi_co_he_so_goc(ham_so, bien, he_so_goc):
    ...
    loi_giai = huong_dan_giai.LoiGiai(
        "Viết phương trình tiếp tuyến với đồ thị hàm số {hs} có hệ số góc là {hsg}".format(
            hs=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(ham_f)),
    ...
    buoc_3 = huong_dan_giai.LoiGiai('Thế các nghiệm tìm được vào ham số, xác định tiếp điểm')
    buoc_3.dap_an = list()
    for nghiem in buoc_2.dap_an:
        thay_b = phuong_trinh.thay_bien(ham_so, bien, nghiem)
        buoc_3.cac_buoc_giai += thay_b.cac_buoc_giai
        buoc_3.them_thao_tac('Vậy ta được tiếp điểm {td}'.format(
            td=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex((nghiem, thay_b.dap_an)))
        ))
        buoc_3.dap_an.append((nghiem, thay_b.dap_an))

    # Buoc 4: Lap phuong trinh tiep tuyen
    buoc_4 = huong_dan_giai.LoiGiai('Lập phương trình tiếp tuyến dựa trên các tiếp điểm')
    ...
    # Buoc 5: Ket luan
    buoc_5 = huong_dan_giai.LoiGiai('Kết luận')
    buoc_5.them_thao_tac('Vậy phương trình tiếp tuyến với đồ thị hàm số có hệ số góc {hsg} là {tt}'.format(
        hsg=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(he_so_goc)),
        tt=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_ngoac_nhon(buoc_4.dap_an))
    ))
...
def viet_phuong_trinh_tiep_tuyen_voi_do_thi_ham_so_di_qua_mot_diem(ham_so,bien,diem):
    ...
    """
    ham_f = phuong_trinh.tao_ham('f',ham_so,bien)
    loi_giai = huong_dan_giai.LoiGiai('Viết phương trình tiếp tuyến với đồ thị hàm số {hs} đi qua điểm {d}'.format(
    ...
    buoc_2.them_thao_tac('Vậy hoành độ của tiếp điểm là nghiệm của phương trình sau {p}'.format(
        d=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(diem)),
        p = xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(pt))
    ))
    # Thay bien vao
    tb = phuong_trinh.thay_bien(pt,[ham_f.lhs,dao_ham_f],[ham_so,buoc_1.dap_an])
    ...
    buoc_3 = huong_dan_giai.LoiGiai('Thay hoành độ vào hàm số tìm tung độ của tiếp điểm')
    tiep_diem = []
    buoc_3.dap_an = []
    for n in buoc_2.dap_an:
        ...
        buoc_3.cac_buoc_giai+=thay_b.cac_buoc_giai
        buoc_3.them_thao_tac('Vậy ta được tiếp điểm {td}'.format(
            td=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex((n,thay_b.dap_an)))
        ))
        tiep_diem.append([n,thay_b.dap_an])
        buoc_3.dap_an.append(thay_b.dap_an)
    # B4: Lap phuong trinh tiep tuyen
    buoc_4 = huong_dan_giai.LoiGiai('Lập phương trình tiếp tuyến tương ứng với các tiếp điểm')
    buoc_4.dap_an=list()
    for td in tiep_diem:
        buoc_4.them_thao_tac('Tiếp tuyến với đồ thị hàm số tại điểm {d} có dạng:'.format(
    ...
    if len(buoc_4.dap_an)==0:
        buoc_5.them_thao_tac('Vậy không có tiếp tuyến thỏa mãn điều kiện đề bài')
    elif len(buoc_4.dap_an)==1:
        buoc_5.them_thao_tac('Vậy tiếp tuyến với đồ thị hàm số đi qua điểm {d} là {tt}'.format(
            d=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(diem)),
            tt=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(buoc_4.dap_an[0]))
        ))
    else:
        buoc_5.them_thao_tac('Vậy các tiếp tuyến với đồ thị hàm số đi qua điểm {d} là {tt}'.format(
            d=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(diem)),
            tt=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_ngoac_nhon(buoc_4.dap_an))
        ))
...
if __name__ == "__main__":
    ...
    def tim_toa_do_giao_diem_voi_duong_thang_test():
        ...
        tim_toa_do_giao_diem_voi_duong_thang(hs, b, d).xuat_html("loi_giai.html")


    def viet_phuong_trinh_tiep_voi_do_thi_ham_so_tai_mot_diem_test():
        hs = sympy.sympify("x**3-2*x+1")
        d = 2
        viet_phuong_trinh_tiep_voi_do_thi_ham_so_tai_mot_diem(hs, b, d).xuat_html("loi_giai.html")


    def viet_phuong_trinh_tiep_tuyen_voi_do_thi_co_he_so_goc_test():
        hs = sympy.sympify("(2*x)/(x-1)")
        hsg = -2
        viet_phuong_trinh_tiep_tuyen_voi_do_thi_co_he_so_goc(hs, b, hsg).xuat_html("loi_giai.html")

    def viet_phuong_trinh_tiep_tuyen_voi_do_thi_ham_so_di_qua_mot_diem_test():
        hs = sympy.sympify("x**3-3*x+1")
        diem = (1,-1)
        viet_phuong_trinh_tiep_tuyen_voi_do_thi_ham_so_di_qua_mot_diem(hs,b,diem).xuat_html("loi_giai.html")
