...
def tim_khong_xac_dinh(ham_so, bien):
    ...
    if phuong_trinh.loai_ham_so(ham_so,
                                bien) in hang_so.LoaiHamSo.CAC_HAM_DA_THUC:
        return []

    # Ham phan thuc khong xac dinh khi mau bang 0
    else:
        if phuong_trinh.loai_ham_so(
                ham_so, bien) in hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC:
...
def tim_tap_xac_dinh(ham_so, bien):
    ...
    if loai_ham in hang_so.LoaiHamSo.CAC_HAM_DA_THUC:
        ...
        loi_giai.them_thao_tac(
            "Hàm số là hàm đa thức nên luôn xác định trên tập số thực")
        loi_giai.them_thao_tac(
            xu_ly_chuoi.boc_mathjax("D={r}".format(r=xu_ly_chuoi.tao_latex(
                txd))))
        loi_giai.dap_an = txd
    ...
    # Ham phan thuc mau khong xac dinh
        ...
        dieu_kien = []
        mau_so = phuong_trinh.lay_mau_so(ham_so)
        loi_giai.them_thao_tac("Mẫu số của hàm số là {0}".format(
            xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(mau_so))))
        ...
        loi_giai.them_thao_tac("Vậy ta có tập xác định {tap_xd}".format(
            tap_xd=xu_ly_chuoi.boc_mathjax("D={txd}".format(
                txd=xu_ly_chuoi.tao_latex(tap_xac_dinh)))))
