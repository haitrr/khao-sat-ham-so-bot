...
def tim_tham_so_de_ham_so_dong_bien_tren_tap_xac_dinh(ham_so, bien, tham_so):
    ...
    loi_giai = huong_dan_giai.LoiGiai(
        "Tìm {0} để hàm số {1} đồng biến trên tập xác định".format(
    ...
    loi_giai.cac_dinh_nghia.append(
        dinh_nghia.DE_HAM_SO_DONG_BIEN_TREN_TAP_XAC_DINH)
    ...
    if ham_so - hs_mau != 0:
        loi_giai.loi_giai_mau = tim_tham_so_de_ham_so_dong_bien_tren_tap_xac_dinh(
    ...
    buoc_3 = phuong_trinh_bac_2.tim_tham_so_de_ham_so_lon_hon_hoac_bang_0(
        buoc_2.dap_an, bien, tham_so)
    buoc_3.ten_loi_giai = 'Tìm tham số để {bdt}'.format(
        bdt=xu_ly_chuoi.boc_mathjax("{dh}\ \\forall \in \mathbb{{R}}").format(
            dh=xu_ly_chuoi.tao_latex(
                bat_dang_thuc.lon_hon_hoac_bang(
                    phuong_trinh.tao_ham("f'", bien, bien).lhs, 0))))
    ...
    if phuong_trinh.so_sanh(buoc_3.dap_an, sympy.EmptySet()):
        buoc_4.them_thao_tac(
            'Vậy hàm số không đồng biến trên tập xác định với bất kỳ giá trị {ts} nào'.
            format(ts=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(tham_so))))
    else:
        buoc_4.them_thao_tac(
            'Vậy hàm số đồng biến trên tập xác định khi {ts}'.format(
                ts=xu_ly_chuoi.boc_mathjax("{ts}\ \in {ng}").format(
...
def tim_tham_so_de_ham_so_nghich_bien_tren_tap_xac_dinh(ham_so, bien, tham_so):
    # De bai
    loi_giai = huong_dan_giai.LoiGiai(
        "Tìm {0} để hàm số {1} nghịch biến trên tập xác định".format(
            xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(tham_so)),
            xu_ly_chuoi.boc_mathjax("f({0})={1}".format(
                xu_ly_chuoi.tao_latex(bien), xu_ly_chuoi.tao_latex(ham_so)))))
    # --------------------------CAU HOI--------------------------
    ch1 = huong_dan_giai.HoiDap(
        'Để hàm số nghịch biến trên tập xác định thì giá trị đạo hàm như thế nào?'
    )
    ch1.cac_goi_y.append('Lớn hơn hay nhỏ hơn 0')
    da1 = huong_dan_giai.DapAnCauHoi('Nhỏ hơn hoặc bằng 0',
                                     ['nho hon', 'bang', ('khong', 0)])
    ...
    # --------------------------DINH NGHIA------------------------
    loi_giai.cac_dinh_nghia.append(
        dinh_nghia.DE_HAM_SO_NGHICH_BIEN_TREN_TAP_XAC_DINH)
    ...
    if ham_so - hs_mau != 0:
        loi_giai.loi_giai_mau = tim_tham_so_de_ham_so_nghich_bien_tren_tap_xac_dinh(
            hs_mau, bien_mau, ts_mau).xuat_html()
    ...
    # Buoc 3 tim tham so de dao ham lon hon hoac bang 0 tren R
    buoc_3 = phuong_trinh_bac_2.tim_tham_so_de_ham_so_nho_hon_hoac_bang_0(
        buoc_2.dap_an, bien, tham_so)
    buoc_3.ten_loi_giai = 'Tìm tham số để {bdt}'.format(
        bdt=xu_ly_chuoi.boc_mathjax("{dh}\ \\forall \in \mathbb{{R}}").format(
            dh=xu_ly_chuoi.tao_latex(
                bat_dang_thuc.nho_hon_hoac_bang(
                    phuong_trinh.tao_ham("f'", bien, bien).lhs, 0))))
    loi_giai.them_thao_tac(buoc_3)
    loi_giai.dap_an = buoc_3.dap_an
    ...
    if phuong_trinh.so_sanh(buoc_3.dap_an, sympy.EmptySet()):
        buoc_4.them_thao_tac(
            'Vậy hàm số không nghịch biến trên tập xác định với bất kỳ giá trị {ts} nào'.
            format(ts=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(tham_so))))
    else:
        buoc_4.them_thao_tac(
            'Vậy hàm số nghịch biến trên tập xác định khi {ts}'.format(
                ts=xu_ly_chuoi.boc_mathjax("{ts}\ \in {ng}").format(
                    ts=xu_ly_chuoi.tao_latex(tham_so),
                    ng=xu_ly_chuoi.tao_latex(buoc_3.dap_an))))
    loi_giai.them_thao_tac(buoc_4)
...
# todo: test
def tim_tham_so_de_ham_so_don_dieu_tren_1_khoang_co_do_dai_k(
        ham_so, bien, tham_so, do_dai_khoang):
    # De bai
    loi_giai = huong_dan_giai.LoiGiai(
        "Tìm {0} để hàm số {1} đơn điệu trên khoảng có độ dài {do_dai}".format(
            xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(tham_so)),
            xu_ly_chuoi.boc_mathjax("f({0})={1}".format(
                xu_ly_chuoi.tao_latex(bien), xu_ly_chuoi.tao_latex(ham_so))),
    ...
    # ------------------------------------------CAU HOI----------------------
    ch1 = huong_dan_giai.HoiDap(
        'Để làm được bài này ta sử dụng định lý nào đã học ?')
    ch1.cac_goi_y.append(
        'Định nghĩa liên quan đến nghiệm của phương trình bậc 2')
    ...
    # -------------------------DINH NGHIA--------------------------------
    loi_giai.cac_dinh_nghia.append(
        dinh_nghia.DE_HAM_SO_DON_DIEU_TREN_MOT_KHOANG_CO_DO_DAI_CHO_TRUOC)
    ...
    if ham_so - hs_mau != 0:
        loi_giai.loi_giai_mau = tim_tham_so_de_ham_so_don_dieu_tren_1_khoang_co_do_dai_k(
            hs_mau, bien_mau, ts_mau, khoang_mau).xuat_html()
    ...
    buoc_2 = huong_dan_giai.LoiGiai(
        'Tìm {ts} để đạo hàm có 2 nghiệm phân biệt và quan hệ giữa 2 nghiệm'.
        format(ts=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(tham_so))))

    # Buoc 2.1: Tim tham so de f' co 2 nghiem phan biet
    buoc_2_1 = phuong_trinh_bac_2.tim_tham_so_de_phuong_trinh_hai_nghiem_phan_biet(
    ...
    dieu_kien = bat_dang_thuc.bang(
        sympy.Abs(nghiem_1 - nghiem_2), do_dai_khoang)
    buoc_3.them_thao_tac(
        'Để hàm số đơn điệu trên khoảng có độ dài {do_dai} thì {dk}'.format(
            do_dai=xu_ly_chuoi.boc_mathjax(
                xu_ly_chuoi.tao_latex(do_dai_khoang)),
            dk=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(dieu_kien))))

    buoc_3.them_thao_tac(
        xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(dieu_kien)))
    dieu_kien = bat_dang_thuc.bang(
        (nghiem_1 + nghiem_2)**2 - 4 * nghiem_1 * nghiem_2, do_dai_khoang**2)
    ...
    buoc_4 = huong_dan_giai.LoiGiai(
        'Thế kết quả từ bước 2 vào bước 3 và giải phương trình tìm nghiệm')
    buoc_4.them_thao_tac(
        xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(dieu_kien)))
    buoc_4.them_thao_tac('Thế kết quả từ bước 2 ta được:')
    # The bien
    dieu_kien = phuong_trinh.the_bieu_thuc(dieu_kien, nghiem_1 + nghiem_2,
                                           buoc_2_2.dap_an[0])
    dieu_kien = phuong_trinh.the_bieu_thuc(dieu_kien, nghiem_1 * nghiem_2,
                                           buoc_2_2.dap_an[1])
    # buoc_4.them_thao_tac(xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(dieu_kien)))

    loi_giai_phuong_trinh = phuong_trinh.giai_phuong_trinh(dieu_kien.lhs,
                                                           tham_so)
    ...
    if thoa_man == []:
        buoc_5.them_thao_tac(
            'Không có {ts} nào thỏa mãn điều kiện đề bài'.format(
                ts=xu_ly_chuoi.boc_mathjax(xu_ly_chuoi.tao_latex(tham_so))))
    else:
        buoc_5.them_thao_tac(
            'Với {ts} thì hàm số {hs} đơn điệu trên khoảng có độ dài {do_dai}'.
            format(
                ts=xu_ly_chuoi.boc_mathjax('{tham_so}={nghiem}'.format(
                    tham_so=xu_ly_chuoi.tao_latex(tham_so),
...
if __name__ == '__main__':
    ...
    def tim_tham_so_de_ham_so_dong_bien_tren_tap_xac_dinh_test():
        ...
        tim_tham_so_de_ham_so_dong_bien_tren_tap_xac_dinh(
            hs, b, ts).xuat_html('loi_giai.html')

    def tim_tham_so_de_ham_so_nghich_bien_tren_tap_xac_dinh_test():
        hs = sympy.sympify("x^3+3*x**2+m*x+m")
        tim_tham_so_de_ham_so_nghich_bien_tren_tap_xac_dinh(
            hs, b, ts).xuat_html('loi_giai.html')
