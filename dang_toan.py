...
class DangToan():
    def __init__(self, khop, ten, can_tham_so, loai_ham_so, ham_giai,
                 cac_du_kien):
        self.khop = khop
        self.ten = ten
...
cac_dang_toan = [
    [
        'Cực trị',
        DangToan(r'^tim tham so de ham so co cuc tri$',
                 'Tìm tham số để hàm số có cực trị', True,
                 hang_so.LoaiHamSo.CAC_HAM_DA_THUC + hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC,
        ...
        DangToan(
            ...
            'Tìm tham số để hàm số có cực trị nằm ở hai phía trục tung', True,
            [hang_so.LoaiHamSo.HAM_BAC_BA],
            cuc_tri.tim_tham_so_de_cuc_tri_nam_o_hai_phia_truc_tung,
            [('ham_so', 'hàm số'), ('bien', 'biến của hàm số'), ('tham_so',
                                                                 'tham số')]),
        DangToan(r'^tim tham so de ham so dat cuc tri tai mot diem cho truoc$',
                 'Tìm tham số để hàm số đạt cực trị tại một điểm cho trước',
                 True, [hang_so.LoaiHamSo.HAM_BAC_BA, hang_so.LoaiHamSo.HAM_BAC_BON],
                 cuc_tri.tim_tham_so_de_ham_so_dat_cuc_tri_tai_mot_diem,
                 [('ham_so', 'hàm số'), ('bien', 'biến của hàm số'),
                  ('tham_so', 'tham số'), ('diem', 'điểm đạt cực trị')]),
        DangToan(
            r'^tim tham so de ham so dat cuc tieu tai mot diem cho truoc$',
            'Tìm tham số để hàm số đạt cực tiểu tại một điểm cho trước', True,
            [hang_so.LoaiHamSo.HAM_BAC_BA, hang_so.LoaiHamSo.HAM_BAC_BON],
            cuc_tri.tim_tham_so_de_ham_so_dat_cuc_tieu_tai_mot_diem,
            [('ham_so', 'hàm số'), ('bien', 'biến của hàm số'),
             ('tham_so', 'tham số'), ('diem', 'điểm đạt cực tiểu')]),
        DangToan(r'^tim tham so de ham so dat cuc dai tai mot diem cho truoc$',
                 'Tìm tham số để hàm số đạt cực đại tại một điểm cho trước',
                 True, [hang_so.LoaiHamSo.HAM_BAC_BA, hang_so.LoaiHamSo.HAM_BAC_BON],
                 cuc_tri.tim_tham_so_de_ham_so_dat_cuc_dai_tai_mot_diem,
                 [('ham_so', 'hàm số'), ('bien', 'biến của hàm số'),
                  ('tham_so', 'tham số'), ('diem', 'điểm đạt cực đại')]),
        DangToan(r'^tim tham so de ham so khong co cuc tri$',
                 'Tìm tham số để hàm số không có cực trị', True,
                 hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC + hang_so.LoaiHamSo.CAC_HAM_DA_THUC,
    ...
    [
        'Đạo hàm',
        DangToan(r'tinh dao ham', 'Tính đạo hàm', False,
                 hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC + hang_so.LoaiHamSo.CAC_HAM_DA_THUC,
    ...
    [
        'Cơ bản',
        DangToan(r'rut gon', 'Rút gọn', False,
                 hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC + hang_so.LoaiHamSo.CAC_HAM_DA_THUC,
                 phuong_trinh.rut_gon_bieu_thuc, [('ham_so', 'hàm số')]),
        DangToan(r'giai phuong trinh', 'Giải phương trình', False,
                 hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC + hang_so.LoaiHamSo.CAC_HAM_DA_THUC,
                 phuong_trinh.giai_phuong_trinh,
                 [('ham_so', 'hàm số'), ('bien', 'biến của hàm số')]),
        DangToan(r'phan tich thanh nhan tu', 'Phân tích thành nhân tử', False,
                 hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC + hang_so.LoaiHamSo.CAC_HAM_DA_THUC,
    ...
    [
        ...
        DangToan(
            ...
            True, [hang_so.LoaiHamSo.HAM_BAC_BA], tinh_don_dieu.
        ...
        DangToan(
            ...
            'Tìm tham số để hàm số nghịch biến trên tập xác định', True,
            [hang_so.LoaiHamSo.HAM_BAC_BA],
        ...
        DangToan(
            ...
            'Tìm tham số để hàm số đồng biến trên tập xác định', True,
            [hang_so.LoaiHamSo.HAM_BAC_BA],
    ...
    [
        'Tương giao đồ thị',
        DangToan(r'^tim giao diem cua do thi ham so voi duong thang$',
                 'Tìm giao điểm của đồ thị hàm số với đường thẳng', False,
                 hang_so.LoaiHamSo.CAC_HAM_DA_THUC + hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC,
        ...
        DangToan(
            ...
            True, [hang_so.LoaiHamSo.HAM_BAC_BA], tuong_giao_do_thi.
            tim_tham_so_de_ham_so_cat_truc_hoanh_tai_1_diem_duy_nhat,
            [('ham_so', 'hàm số'), ('bien', 'biến của hàm số'), ('tham_so',
                                                                 'tham số')]),
        DangToan(
            r'^viet phuong trinh tiep tuyen voi do thi ham so tai mot diem cho truoc',
        ...
        DangToan(
            ...
            False, hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC + hang_so.LoaiHamSo.CAC_HAM_DA_THUC,
            tuong_giao_do_thi.
            viet_phuong_trinh_tiep_tuyen_voi_do_thi_co_he_so_goc, [
                ('ham_so', 'hàm số'), ('bien', 'biến của hàm số'),
                ('he_so_goc', 'hệ số góc của tiếp tuyến')
            ]),
        DangToan(
            ...
            viet_phuong_trinh_tiep_tuyen_voi_do_thi_ham_so_di_qua_mot_diem, [
                ('ham_so', 'hàm số'), ('bien', 'biến của hàm số'),
                ('diem', 'điểm mà tiếp tuyến đi qua')
            ]),
    ],
    [
        "Giá trị lớn nhất,nhỏ nhất",
        DangToan(r'^tim gia tri lon nhat va nho nhat cua ham so$',
                 'Tìm giá trị lớn nhất và nhỏ nhất của hàm số', False,
                 hang_so.LoaiHamSo.CAC_HAM_DA_THUC + hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC,
        ...
        DangToan(
            ...
            False, hang_so.LoaiHamSo.CAC_HAM_DA_THUC + hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC,
            gia_tri.
            tim_gia_tri_lon_nhat_va_nho_nhat_cua_ham_so_trong_mot_khoang_cho_truoc,
            [('ham_so', 'hàm số'), ('bien', 'biến của hàm số'), ('khoang',
                                                                 'khoảng')]),
    ],
    DangToan(r'^khao sat va ve do thi ham so$', 'Khảo sát và vẽ đồ thị hàm số',
             False, hang_so.LoaiHamSo.CAC_HAM_PHAN_THUC + hang_so.LoaiHamSo.CAC_HAM_DA_THUC,
